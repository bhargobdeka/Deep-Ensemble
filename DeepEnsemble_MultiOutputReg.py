...
class Net(nn.Module):
    def __init__(self, input_dim, output_dim, num_units):
        ...
        self.l2 = nn.Linear(num_units, 4) #3-output units and 3 variance units
...
def fit_model(model, optimizer, train_loader):
    ...
    for x_batch, y_batch in train_loader:
        ...
        mu, sig = output[:,[0,1]], sp(output[:,[2,3]])+(10)**-6
        ...
        output_a    = model(x_a)
        mu_a, sig_a = output_a[:,[0,1]], sp(output_a[:,[2,3]])+(10)**-6
...
#    call(["rm", "results/log_{}.txt".format(data_name[j])], shell=True)
    ...
    _RESULTS_RMSEtest = data_name[j] + "/results_DE/RMSEtest_DE.txt"
    ...
    for i in range(n_splits):
        ...
        for epoch in range(num_epochs):
            ...
            for model, op in models:
                # start_time_oneE = time.time()
                model = fit_model(model, op, train_loader)
                # runtime_oneE = time.time() - start_time_oneE
                # print("runtime for one Epoch: "+str(runtime_oneE))
        ...
        # runtimes_oneE = time.time()-start_time
        lltests.append(lls)
        rmsetests.append(errors)
        runtimes.append(runtime)
        # runtimes_oneE.append(runtimes_oneE)
    ...
    # with open("results_DE/log_oneEpoch_{}.txt".format(data_name[j]), "a") as myfile:
    #            myfile.write('Avg. runtime for one Epoch is %f +- %f  \n' % (np.mean(runtimes_oneE), np.std(runtimes_oneE)))
